...
submit_button.addEventListener('click', () => {
    ...
    if (isImageBlank(img)) {
        message.innerHTML = '<span style="color:red">Error</span><br/>Тут пусто...';
        return;
    }
    ...
    .then(res=>res.json())
    .then( res => {
        message.innerHTML = (res.digit !== undefined) ?
            'Digit: ' + res.digit + '<br/>':
            '<span style="color:blueviolet">Error</span><br/>Unexpected response';
    }).catch(err => {
        message.innerHTML = '<span style="color:red">Ошибочка</span><br/>'
        console.log(err);
    });
});
